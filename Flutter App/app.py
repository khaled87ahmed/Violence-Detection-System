...
try:
    model = YOLO("violence_detection_model.pt")
        ...
        for result in results:
            if len(result.boxes) > 0:
                for box in result.boxes:
                    ...
                    if (is_violence and confidence >= 0.1) or (not is_violence and confidence >= 0.4):
                        color = (0, 0, 255) if is_violence else (0, 255, 0)
                        if is_violence and confidence >= 0.1:
                            violence_detected = True
                        x1, y1, x2, y2 = map(int, box.xyxy[0])
                        cv2.rectangle(annotated_frame, (x1, y1), (x2, y2), color, 10)
                        label = f"{class_name} {confidence:.2f}"
                        cv2.putText(annotated_frame, label, (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 3)
        return violence_detected, annotated_frame, frame if violence_detected else None
    except Exception as e:
...
def generate_camera_frames():
    ...
    with camera_lock:
        ...
        while CAMERA_ON:
            ...
            if not success:
                break
            violence, processed, alert_frame = detect_violence(frame)
            now = time.time()
            if violence and (now - last_email_time > cooldown):
                Thread(target=send_violence_alert, args=(processed, now)).start()
                last_email_time = now
                socketio.emit('violence_detected', {
                    'timestamp': now,
                    'message': 'Violence detected! Email sent.'
                })
...
def upload_video():
    ...
    def process_task():
        success, violence_percent = process_video(input_path, output_path)
        if success:
            socketio.emit('processing_complete', {
                'original_video': f'/uploads/{video_id}_{filename}',
                'processed_video': f'/processed/processed_{video_id}_{filename}',
                'violence_percentage': round(violence_percent, 2)
            })
        else:
            socketio.emit('processing_error', {'error': 'Processing failed'})
...
def process_video(input_path, output_path):
    try:
        ...
        while cap.isOpened():
            ...
            if not ret:
                break
            violence, processed, _ = detect_violence(frame)
            if violence:
                violence_frames += 1
            out.write(processed)
            current_frame += 1
        ...
        cap.release()
        out.release()
        violence_percent = (violence_frames / frame_count) * 100 if frame_count else 0
        return True, violence_percent
    except Exception as e:
...
@app.route('/status')
def status():
    return jsonify({'model_loaded': model is not None, 'camera_active': CAMERA_ON})
